...
def _force_egress_fail(monkeypatch):
    """Make the UDP-egress probe raise so _primary_ipv4 falls through to the
    interface-scan path (the branch that must skip docker0/bridges)."""
    class _Boom:
        def __init__(self, *a, **k):
            raise OSError('no route')
    monkeypatch.setattr(app.socket, 'socket', _Boom)


def test_primary_ipv4_skips_docker_and_bridge(monkeypatch):
    # Regression: a Docker host listed docker0 (172.17.0.1) / br-* (172.18.0.1)
    # last, and the old code returned the LAST address it saw — reporting the
    # Docker bridge IP instead of the real LAN IP.
    _force_egress_fail(monkeypatch)
    addr = ('[{"ifname":"lo","addr_info":[{"family":"inet","local":"127.0.0.1"}]},'
            '{"ifname":"eth0","addr_info":[{"family":"inet","local":"192.168.1.10"}]},'
            '{"ifname":"br-abc","addr_info":[{"family":"inet","local":"172.18.0.1"}]},'
            '{"ifname":"docker0","addr_info":[{"family":"inet","local":"172.17.0.1"}]}]')

    def fake_run(args, input_data=None, no_sudo=False):
        if 'route' in args:
            return '[]', '', 0                 # no default route → interface scan
        return addr, '', 0

    monkeypatch.setattr(app, 'run', fake_run)
    assert app._primary_ipv4() == '192.168.1.10'


def test_primary_ipv4_prefers_default_route_dev(monkeypatch):
    _force_egress_fail(monkeypatch)

    def fake_run(args, input_data=None, no_sudo=False):
        if 'route' in args:
            return '[{"dst":"default","dev":"eth0","gateway":"192.168.1.1"}]', '', 0
        # addr show for that dev
        return '[{"ifname":"eth0","addr_info":[{"family":"inet","local":"192.168.1.10"}]}]', '', 0
...
def test_hostname_and_domain_regexes():
    assert app.RE_HOST_LABEL.match('node1')
...
def test_render_netplan_static_with_gateway_and_dns():
    conf = {'ethernets': {'ens18': {'dhcp4': False, 'addresses': ['192.168.10.50/24'],
                                     'gateway': '192.168.10.1', 'nameservers': ['1.1.1.1', '8.8.8.8']}},
            'bridges': {}}
    yaml = app.render_netplan(conf)
    assert 'dhcp4: false' in yaml
    assert '- 192.168.10.50/24' in yaml
    assert 'to: default' in yaml
    assert 'via: 192.168.10.1' in yaml
...
def test_union_does_not_relist_dhcp_lease_as_static():
    ...
    old = {'dhcp4': True, 'addresses': ['192.168.10.88/23']}
    dual = app._net_union_spec(old, {'dhcp4': False, 'addresses': ['192.168.10.200/23']})
    assert dual['dhcp4'] is True
    assert dual['addresses'] == ['192.168.10.200/23']  # only the new static
