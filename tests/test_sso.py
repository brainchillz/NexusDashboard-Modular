...
from nexusdash.core import sso
...
def test_callback_is_inert_when_unconfigured():
    """The callback is registered unconditionally — it has to be, so a UI
    enrollment applies without a restart (Flask cannot add a route to a running
    app; the module system solves it the same way). Unconfigured it must behave
    as though it were absent."""
    import app
    c = app.app.test_client()
    r = c.get('/sso/callback?a=anything')
    assert r.status_code == 404
    # And it grants nothing: no session cookie comes back.
    assert not any('session' in v for v in r.headers.getlist('Set-Cookie'))


def test_enrollment_endpoints_refuse_anonymous_callers():
    import app
    c = app.app.test_client()
    assert c.get('/api/sso').status_code == 401
    assert c.post('/api/sso/enroll', json={'issuer': 'https://x', 'code': 'y'}
                  ).status_code == 401
    assert c.delete('/api/sso', json={'password': 'x'}).status_code == 401
