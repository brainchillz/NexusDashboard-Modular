...
def test_scan_finds_every_module_file():
    ...
    assert names == {
        ...
        'containers.console', 'updates', 'nut', 'upsmon', 'halogen', 'docker_registry'}
...
def test_legacy_facade_order_reproduced():
    ...
    # it alphabetically (docker_registry < halogen < nut < updates < upsmon).
    assert tuple(names) == discovery._LEGACY_FACADE_ORDER + ('docker_registry', 'halogen',
                                                             'nut', 'updates', 'upsmon')
